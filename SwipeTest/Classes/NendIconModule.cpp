...
//IconViewの生成(位置指定)
void NendIconModule::createNADIconView(CCPoint pos){
	cocos2d::JniMethodInfo t;
...
extern "C"
{
	JNIEXPORT void JNICALL Java_net_nend_NendModule_NendIconModule_onReceiveIconAd(JNIEnv* env, jobject thiz) {
	    //初回ロード終了のオブザーバー通知
	    CCNotificationCenter::sharedNotificationCenter()->postNotification("NEND_ICON_NOTIFICATION_RECEIVE_AD", NULL);
	}

	JNIEXPORT void JNICALL Java_net_nend_NendModule_NendIconModule_onFailedIconAd(JNIEnv* env, jobject thiz) {
	    //ロード失敗のオブザーバー通知
	    CCNotificationCenter::sharedNotificationCenter()->postNotification("NEND_ICON_NOTIFICATION_FAIL_TO_RECEIVE_AD", NULL);
	}

	JNIEXPORT void JNICALL Java_net_nend_NendModule_NendIconModule_onClickIconAd(JNIEnv* env, jobject thiz) {
	    //広告クリックのオブザーバー通知
	    CCNotificationCenter::sharedNotificationCenter()->postNotification("NEND_ICON_NOTIFICATION_CLICK_AD", NULL);
	}
}
